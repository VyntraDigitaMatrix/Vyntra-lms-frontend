...
const DiscussionCard = ({ discussion, onOpen, loading }) => {
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full text-left bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-violet-200 transition-all duration-200 p-4 sm:p-5 flex items-center gap-4 disabled:opacity-70"
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {discussion.latestMessage ? (
          <p className="text-xs text-gray-500 line-clamp-1 mt-0.5">{discussion.latestMessage}</p>
        ) : (
