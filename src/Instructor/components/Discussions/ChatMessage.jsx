  ...
  return (
    ...
    >
      <img
        ...
        alt={msg.senderName}
        className="w-8 h-8 rounded-full object-cover border border-slate-200 shadow-sm flex-shrink-0 self-end"
      />
      ...
      <div className={`flex flex-col max-w-[72%] sm:max-w-[60%] ${isOwn ? "items-end" : "items-start"}`}>
        <div className={`flex items-center gap-1.5 mb-1 ${isOwn ? "mr-1 flex-row-reverse" : "ml-1"}`}>
          <span className="text-xs font-semibold text-slate-700">{isOwn ? "You" : msg.senderName}</span>
          <RoleBadge role={msg.senderRole} />
        </div>

        {msg.replyToMessageId && (
          <div className={`text-[11px] px-3 py-1.5 rounded-xl mb-1 border-l-2 ${isOwn ? "border-blue-400 bg-blue-50 text-blue-900" : "border-slate-300 bg-slate-100 text-slate-600"}`}>
        ...
        <div className={`relative px-4 py-2.5 rounded-2xl text-sm leading-relaxed shadow-sm
          ${isOwn ? "bg-[#043573] text-white rounded-br-sm" : "bg-white border border-slate-200 text-slate-800 rounded-bl-sm"}
        ...
        `}>
          ...
          {msg.edited && !msg.deleted && (
            <span className={`text-[9px] ml-1 ${isOwn ? "text-blue-200" : "text-slate-400"}`}>(edited)</span>
          )}
        </div>
        ...
        <div className={`flex items-center gap-2 mt-1 ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
          ...
          {msg.seenCount > 0 && isOwn && (
            <span className="flex items-center gap-0.5 text-[10px] text-[#043573]">
      ...
      {hovered && !msg.deleted && (
        <div className={`flex items-center gap-1 self-center ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
          <button onClick={() => onReply(msg)} className="p-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 transition" title="Reply">
            <FaReply size={10} />
          </button>
