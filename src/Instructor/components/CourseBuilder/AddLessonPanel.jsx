...
function AddLessonPanel({ onClose, onSave }) {
    ...
    const [resourceInputType, setResourceInputType] = useState("FILE_UPLOAD");
    ...
    const canSave =
        ...
        (
            lessonType === "VIDEO" 
                ...
                : (
                    (resourceInputType === "FILE_UPLOAD"
                        ? resourceFile
                        : resourceInputType === "URL"
                            ? resourceUrl.trim()
                            : true) // Allow true if no resource is strictly required for other types, or force it if desired
                )
        );
    ...
    const handleSave = () => {
        ...
        // Thumbnail and Video only for VIDEO lesson
        if (lessonType === "VIDEO") {
            // Thumbnail
            ...
            // Video
            if (videoInputType) {
                formData.append("videoInputType", videoInputType);
            ...
            // Resources for non-video lessons
            if (resourceInputType) {
                formData.append("resourceInputType", resourceInputType);
                if (resourceInputType === "URL" && resourceUrl) {
                    formData.append("resourceUrl", resourceUrl);
                } else if (resourceInputType === "FILE_UPLOAD" && resourceFile) {
    ...
    return (
        ...
        >
            <div className="space-y-5">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        if (t.id !== "VIDEO") {
                                            ...
                                            // Set defaults when VIDEO is selected
                ...
                {/* Media Links (Thumbnail and Video URL) for VIDEO type */}
                {lessonType === "VIDEO" && (
                    <>
                        {/* Thumbnail */}
                        <div>
                ...
                {/* Resource for NON-VIDEO type */}
                {lessonType !== "VIDEO" && (
                    <div>
                        <label className="text-sm font-semibold text-gray-700 block mb-1.5">
                            Resource <span className="text-gray-400 font-normal">(Required)</span>
                        </label>
                        <div className="space-y-3">
                            <select
                                value={resourceInputType || ""}
                                onChange={(e) => {
                                    setResourceInputType(e.target.value);
                                    if (e.target.value !== "FILE_UPLOAD") {
                            ...
                            {resourceInputType === "FILE_UPLOAD" && (
                                <div>
                                    ...
                                    {resourceFile && (
                                        <div className="mt-2">
                                            <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg border border-gray-200">
                                                <span className="text-sm text-gray-700">{resourceFile.name}</span>
                                                <span className="text-xs text-gray-400 ml-auto">
                            ...
                            {resourceInputType === "URL" && (
                                <div>
                                    <input
                                        ...
                                        placeholder="Enter resource URL (e.g. https://example.com/file.pdf)"
                                        className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-800 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition"
                                    />
                                </div>
                            )}
