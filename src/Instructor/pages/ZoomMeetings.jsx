...
function Toast({ msg, type, onClose }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl shadow-xl text-white text-sm font-semibold flex items-center gap-3 animate-bounce ${
      ...
      <span>{msg}</span>
      <button onClick={onClose} className="text-white hover:text-slate-200 font-bold ml-2">&times;</button>
    </div>
  );
}
...
function ScheduleMeetingModal({ onClose, onSaved, coursesList, initialCourseSlug }) {
  ...
  const inp =
    "w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition bg-white";
  const lbl =
    "text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5 block";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col animate-scale-up">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-blue-100 text-[#043573] flex items-center justify-center">
              <MdVideoCall size={20} />
            </div>
            <h2 className="text-lg font-bold text-slate-900">Schedule Zoom Meeting</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-2.5 bg-[#043573] hover:bg-blue-900 disabled:opacity-55 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl transition flex items-center justify-center gap-2 shadow-sm"
          >
            {saving ? (
              <>
          ...
          <button
            onClick={onClose}
            disabled={saving}
            className="flex-1 py-2.5 border border-slate-200 bg-white text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-50 transition"
          >
            Cancel
          </button>
        </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="bg-white border-b border-slate-100 px-8 py-5">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#7c3aed] text-white flex items-center justify-center shadow-md">
              <FaLaptop size={20} />
            </div>
            <div>
              <h1 className="text-xl font-bold text-slate-900">Zoom Live Classes</h1>
              <p className="text-xs text-slate-500 mt-0.5">Schedule, configure and host live webinars and sessions.</p>
            </div>
          </div>
          <button
            onClick={() => setModalOpen(true)}
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#7c3aed] hover:bg-violet-750 text-white text-sm font-bold rounded-xl transition shadow-md hover:shadow-lg self-start sm:self-center"
          >
      ...
      <div className="flex-1 px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: "Total Sessions", value: stats.total, bg: "bg-violet-50", text: "text-violet-900", border: "border-t-[#7c3aed]", icon: "💻" },
            ...
            <div
              key={label}
              className={`bg-white rounded-xl border border-slate-200 ${border} border-t-2 p-4 shadow-sm flex items-center gap-3.5`}
            >
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex-1 min-w-[200px] max-w-sm">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-violet-400 focus:ring-1 focus:ring-violet-200 bg-white"
            >
              {courses.map((c) => (
                <option key={c.slug || c.id} value={c.slug || c.id}>
                  {c.title}
          ...
          <div className="flex flex-1 items-center gap-2.5 max-w-md w-full border border-slate-200 rounded-lg px-3 py-2 focus-within:border-violet-400 transition bg-white ml-auto">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <AiOutlineLoading3Quarters className="animate-spin text-3xl text-[#7c3aed]" />
            <p className="text-sm font-semibold text-slate-500">Loading live classes...</p>
          </div>
        ) : filteredMeetings.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl py-16 px-4 text-center shadow-sm flex flex-col items-center justify-center max-w-md mx-auto">
            <div className="w-16 h-16 rounded-full bg-violet-50 text-[#7c3aed] flex items-center justify-center mb-4">
            ...
            <button
              onClick={() => setModalOpen(true)}
              className="mt-5 flex items-center gap-1.5 px-4 py-2 bg-[#7c3aed] hover:bg-violet-750 text-white text-xs font-bold rounded-xl transition shadow-md"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredMeetings.map((meeting) => {
              ...
              return (
                <div
                  key={meeting.id}
                  className="bg-white border border-slate-200 hover:border-violet-300 rounded-2xl p-5 shadow-sm hover:shadow-md transition flex flex-col justify-between"
                >
                  <div className="space-y-4">
                    {/* Status & Breadcrumbs */}
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex flex-wrap gap-1 items-center text-[10px] text-slate-400 font-semibold tracking-wider uppercase">
                        <span>{meeting.courseName || "Course"}</span>
                        {meeting.moduleName && (
                          <>
                            <FaChevronRight className="text-[7px]" />
                            <span className="text-slate-500">{meeting.moduleName}</span>
                          </>
                        )}
                        {meeting.lessonName && (
                          <>
                            <FaChevronRight className="text-[7px]" />
                            <span className="text-[#7c3aed]">{meeting.lessonName}</span>
                          </>
                        )}
                      </div>
                    ...
                    {/* Time Details */}
                    <div className="flex flex-wrap gap-x-5 gap-y-2 bg-slate-50 border border-slate-100 rounded-xl p-3 text-xs text-slate-600">
                      <div className="flex items-center gap-1.5 font-medium">
                        <MdCalendarToday className="text-[#7c3aed] text-sm" />
                        <span>{formattedDate}</span>
                      </div>
                      <div className="flex items-center gap-1.5 font-medium">
                        <MdAccessTime className="text-[#7c3aed] text-sm" />
                        <span>{formattedTime} ({meeting.duration} Mins)</span>
                      </div>
                    </div>

                    {/* Join credentials */}
                    <div className="grid grid-cols-2 gap-3.5 bg-slate-50 border border-slate-100 rounded-xl p-3 text-[11px] text-slate-700 font-semibold">
                      <div className="flex flex-col gap-1 min-w-0">
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                          Meeting ID
                        </span>
                        <div className="flex items-center gap-1 min-w-0">
                          <MdMeetingRoom size={13} className="text-slate-400 flex-shrink-0" />
                      ...
                      <div className="flex flex-col gap-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 min-w-0">
                          <MdVpnKey size={13} className="text-slate-400 flex-shrink-0" />
                  ...
                  <div className="flex items-center gap-2 mt-5 pt-4 border-t border-slate-100">
                    <button
                      ...
                      className="flex-1 py-2 bg-[#7c3aed] hover:bg-violet-750 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed text-white text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 shadow-sm"
                    >
                      <MdVideoCall size={16} />
                      Start Meeting
                    </button>
                    <button
                      ...
                      className="flex-1 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 bg-white"
                    >
                      <MdLink size={16} />
                      Copy Join Link
                    </button>
