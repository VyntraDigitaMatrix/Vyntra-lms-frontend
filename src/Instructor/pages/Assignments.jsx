    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">{isEdit ? "Edit Assignment" : "Create Assignment"}</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500"><MdClose size={20} /></button>
        </div>
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-8 z-20 bg-white border border-gray-100 rounded-xl shadow-xl min-w-[170px] py-1 text-sm">
                ...
                {status === "DRAFT" && <button onClick={() => { onPublish(a); setMenuOpen(false); }} className="flex items-center gap-2 w-full px-4 py-2 hover:bg-emerald-50 text-emerald-700 transition"><MdPublish size={15} /> Publish</button>}
...
function SubmissionsPanel({ assignment, onClose }) {
  ...
  const handleGrade = async (subId) => {
    if (!gradeVal) return;
    try {
      await instructorAssignmentApi.grade(subId, { marks: Number(gradeVal) });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div><h2 className="text-lg font-bold text-gray-900">Submissions</h2><p className="text-xs text-gray-400">{assignment.title}</p></div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500"><MdClose size={20} /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading ? (
            ...
            <div className="space-y-3">
              {submissions.map(sub => (
                <div key={sub.id} className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {sub.marks != null ? (
                        <span className="text-sm font-bold text-emerald-600">{sub.marks}/{assignment.maxMarks}</span>
                      ) : grading === sub.id ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => handleGrade(sub.id)} className="px-3 py-1 bg-violet-600 text-white text-xs rounded-lg hover:bg-violet-700">Save</button>
                          <button onClick={() => setGrading(null)} className="text-gray-400 hover:text-gray-600"><MdClose /></button>
                        </div>
                      ) : (
                        <button onClick={() => { setGrading(sub.id); setGradeVal(""); }}
...
export default function Assignments() {
  ...
  const loadAssignments = useCallback(async (lessonSlug) => {
    ...
    try {
      let res;
      if (lessonSlug) {
        res = await instructorAssignmentApi.getByLesson(lessonSlug, 0, 100);
      } else {
        res = await instructorAssignmentApi.getAllAssignments(0, 100);
      }
      setAssignments(extractList(res));
    } catch (err) {
      console.error(err);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(a => (
              <AssignmentCard key={a.id} a={a}
                ...
                onArchive={handleArchive} onViewSubmissions={item => setViewSubs(item)} />
            ))}
          </div>
        )}
      </div>
      ...
      {modalOpen && (
        <AssignmentModal editData={editData}
          ...
          onSaved={() => {
            ...
            if (selectedLesson?.slug) loadAssignments(selectedLesson.slug);
          }} />
      )}
      {viewSubs && <SubmissionsPanel assignment={viewSubs} onClose={() => setViewSubs(null)} />}
