  ...
  return (
    <>
      ...
      >
        ...
        <a href="/UserLogin" style={{ fontFamily: 'Inter', fontWeight: 400, fontStyle: 'Regular', fontSize: '11px', color: '#000000' }} className="flex items-center gap-1 bg-[#F5A624] hover:bg-amber-600 px-3 py-1 rounded-full transition-colors">
          Check it out
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 mb-10 z-10">
        <button onClick={() => window.location.href = '/UserLogin'} style={{ fontFamily: 'Inter', fontWeight: 700, fontStyle: 'Bold', fontSize: '13px', color: '#FFFFFF' }} className="bg-[#0A2647] hover:bg-[#1E2B43] px-6 py-3 rounded-full shadow-lg transition-all w-full sm:w-auto">
          Start Your Journey
        </button>
        <button onClick={() => { document.getElementById('progress')?.scrollIntoView({ behavior: 'smooth' }); window.history.pushState(null, '', '/#progress'); }} style={{ fontFamily: 'Inter', fontWeight: 700, fontStyle: 'Bold', fontSize: '13px', color: '#0A2647' }} className="flex items-center justify-center gap-2 bg-transparent border border-[#1B3459] hover:[#1B3459]/50 px-6 py-3 rounded-full transition-all w-full sm:w-auto">
